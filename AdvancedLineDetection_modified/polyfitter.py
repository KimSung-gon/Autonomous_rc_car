...
class Polyfitter:
    ...
    def polyfit(self, img, max_y):
        ...
        # draw line
        pts_left = np.array([[width/2 - 10, height], [width/2 - 10, height - 40], [width/2 - 160, height - 180], [width/2 - 160, height]], np.int32)
        cv2.polylines(out_img, [pts_left], False, (0,255,0), 1)
        pts_right = np.array([[width/2 + 10, height], [width/2 + 10, height - 60], [width/2 + 160, height - 180], [width/2 + 160, height]], np.int32)
        ...
        good_left_inds = ((nonzeroy >= nonzerox * 0.93 + 151) & (nonzerox <= width/2 - 10)).nonzero()[0]
        #good_right_inds = ((nonzeroy >= nonzerox * (-0.8) + 684) & (nonzerox >= width/2 + 10)).nonzero()[0]
        ...
        if line_flag != 3:
            ...
            for window in range(0, nwindows):
                ...
                if win_y_low >= 338 and win_y_low < 344:
                    # 0.115 is the half of the road(0.23)
        ...
        return out_img, max_y, x_location
