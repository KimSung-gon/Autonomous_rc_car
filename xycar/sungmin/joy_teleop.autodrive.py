...
class JoyTeleop:
    ...
    def __init__(self):
        ...
        self.mapping = ["drive.steering_angle","drive.speed"]
     ...
     # XYCAR================================================
        ...
        self.car_angle_speed = 0.05
	self.car_angle = 0
	self.car_run_speed = 0.0
	self.car_direction = 0

	rospy.Timer(rospy.Duration(0.01),self.management_run)

    # XYCAR================================================

    def management_run(self,evt=None):

        self.after = datetime.datetime.now()
        self.timedelta = (self.after - self.before).seconds

	if self.timedelta < 3:
            self.auto_run("front","none")
	elif self.timedelta < 6:
	    self.auto_run("front","left")
	elif self.timedelta < 7:
	    self.auto_run("front","none")
	elif self.timedelta < 10:
	    self.auto_run("front","right")
	elif self.timedelta < 12:
            self.auto_run("none","none")
	elif self.timedelta < 15:
	    self.auto_run("back","none")
	elif self.timedelta < 18:
	    self.auto_run("back","right")
	elif self.timedelta < 19:
	    self.auto_run("back","none")
	elif self.timedelta < 22:
	    self.auto_run("back","left")
	elif self.timedelta < 24:
	    self.auto_run("none","none")
        else:
            self.before = self.after
        

    def auto_run(self,front_back,left_right):
        try:
	    rospy.logerr(self.car_run_speed)
            if front_back.upper() == "FRONT":
		if self.car_run_speed < 5.0:
			self.car_run_speed += 0.01
                        # time.sleep(0.1)
		else:
			self.car_run_speed = 2.0
            elif front_back.upper() == "BACK":
		if self.car_run_speed > -5.0:
			self.car_run_speed -= 0.01
		else:
			self.car_run_speed = -2.0
            elif front_back.upper() == "NONE":
            # else:
		if self.car_run_speed > 0:
			self.car_run_speed -= 0.01
		elif self.car_run_speed < 0:
			self.car_run_speed += 0.01
		else:
			self.car_run_speed = 0

            if left_right.upper() == "LEFT":
		if self.car_angle_speed < 0.2:
			self.car_angle_speed +=0.01
		else:
			self.car_angle_speed = 0.2
                self.car_angle = 1
		...
		if self.car_angle_speed < 0.2:
			...
			self.car_angle_speed = 0.2
                self.car_angle = -1
            elif left_right.upper() == "NONE":
            # else:
		if self.car_angle > 0:
			self.car_angle -= 0.1
		elif self.car_angle < 0:
			self.car_angle += 0.1
		if self.car_angle_speed > 0.05:
			self.car_angle_speed -= 0.01
		else:
			self.car_angle_speed = 0.05


            msg = self.get_message_type("ackermann_msgs/AckermannDriveStamped")()
           
            self.set_member(msg,self.mapping[1], self.car_run_speed)
            self.publishers['low_level/ackermann_cmd_mux/input/teleop'].publish(msg)
            self.set_member(msg,self.mapping[0],self.car_angle * self.car_angle_speed)
            self.publishers['low_level/ackermann_cmd_mux/input/teleop'].publish(msg)
        except:
            rospy.logerr("ERR")
